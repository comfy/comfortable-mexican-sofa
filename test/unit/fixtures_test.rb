...
#     ComfortableMexicanSofa::Fixture::Layout::Exporter.new('sample-site', 'default-site').export!
...
#     assert File.exists?(attr_path), "attr path failed"
#     assert File.exists?(content_path), "content path failed"
#     assert_equal ({'label' => 'Default Snippet'}), YAML.load_file(attr_path), "load file failed"
#     assert_equal cms_snippets(:default).content, IO.read(content_path), "io.read failed"
...
#     assert File.exists?(attr_path), :message => "attr path dosen't exist"
#     assert File.exists?(file_path), :message => "file path dosen't exist"
...
#     f = Cms::File.find_by_label('Default File')
#     f.file = File.open(Rails.root.join("test", "fixtures", "files", "image.jpg"))
#     f.save!
