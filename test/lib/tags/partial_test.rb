...
class PartialTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{ cms:partial:partial_name }}'
    )
    assert_equal 'partial_name', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{cms:partial:path/to/partial}}'
    )
    assert_equal 'path/to/partial', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{cms:partial:path/to/dashed-partial}}'
    )
  ...
  def test_initialize_tag_with_parameters
    assert tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{cms:partial:path/to/partial:param1:param2}}'
    )
  ...
  def test_content_and_render
    ...
    tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{cms:partial:path/to/partial:param1}}'
    )
  ...
  def test_whitelisted_paths
    ...
    tag = ComfortableMexicanSofa::Tag::Partial.initialize_tag(
      comfy_cms_pages(:default), '{{cms:partial:safe/path}}'
    )
