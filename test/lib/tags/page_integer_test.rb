...
class PageIntegerTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::PageInteger.initialize_tag(
      comfy_cms_pages(:default), '{{ cms:page:content:integer }}'
    )
    ...
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageInteger.initialize_tag(
      comfy_cms_pages(:default), '{{cms:page:dash-content:integer}}'
    )
    assert_equal 'dash-content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageInteger.initialize_tag(
      comfy_cms_pages(:default), '{{cms:page:namespace.content:integer}}'
    )
