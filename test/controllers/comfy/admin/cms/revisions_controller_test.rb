...
class Comfy::Admin::Cms::RevisionsControllerTest < ActionController::TestCase
  ...
  def test_get_index_for_page_translations
    get :index, :site_id => comfy_cms_sites(:default), :page_id => comfy_cms_pages(:default), :translation_id => comfy_cms_page_translations(:default)
    assert_response :redirect
  ...
  def test_get_show_for_page_translation
    get :show, :site_id => comfy_cms_sites(:default), :page_id => comfy_cms_pages(:default), :translation_id => comfy_cms_page_translations(:default), :id => comfy_cms_revisions(:translation)
    assert_response :success
    assert assigns(:record)
    assert assigns(:revision)
    assert assigns(:record).is_a?(Comfy::Cms::Page::Translation)
    assert_template :show
  end
  ...
  def test_get_show_for_page_translation_failure
    site = comfy_cms_sites(:default)
    page = comfy_cms_pages(:default)
    get :show, :site_id => site, :page_id => page, :translation_id => comfy_cms_page_translations(:default), :id => 'invalid'
    assert_response :redirect
    assert assigns(:record)
  ...
  def test_revert_for_page_translation
    page = comfy_cms_pages(:default)
    translation = comfy_cms_page_translations(:default)

    assert_difference 'translation.revisions.count' do
      put :revert, :site_id => comfy_cms_sites(:default), :page_id => page, :translation_id => translation, :id => comfy_cms_revisions(:translation)
      assert_response :redirect
      assert_redirected_to edit_comfy_admin_cms_site_page_translation_path(page.site, page, translation)
      assert_equal 'Content Reverted', flash[:success]

      translation.reload
      assert_equal [
        { :identifier => 'default_translation_text',  :content => 'revision translation content'   }
      ], translation.blocks_attributes
    end
  end
