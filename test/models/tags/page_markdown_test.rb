...
class PageMarkdownTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::PageMarkdown.initialize_tag(
      cms_page_contents(:default), '{{ cms:page:content:markdown }}'
    )
    ...
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageMarkdown.initialize_tag(
      cms_page_contents(:default), '{{cms:page:dash-content:markdown}}'
    )
    assert_equal 'dash-content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageMarkdown.initialize_tag(
      cms_page_contents(:default), '{{cms:page:namespace.content:markdown}}'
    )
