...
class PageRichTextTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::PageRichText.initialize_tag(
      cms_page_contents(:default), '{{ cms:page:content:rich_text }}'
    )
    ...
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageRichText.initialize_tag(
      cms_page_contents(:default), '{{cms:page:dash-content:rich_text}}'
    )
    assert_equal 'dash-content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::PageRichText.initialize_tag(
      cms_page_contents(:default), '{{cms:page:namespace.content:rich_text}}'
    )
