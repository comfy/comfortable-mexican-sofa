...
class FieldIntegerTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::FieldInteger.initialize_tag(
      cms_page_contents(:default), '{{ cms:field:content:integer }}'
    )
    ...
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::FieldInteger.initialize_tag(
      cms_page_contents(:default), '{{cms:field:dash-content:integer}}'
    )
    assert_equal 'dash-content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::FieldInteger.initialize_tag(
      cms_page_contents(:default), '{{cms:field:namespace.content:integer}}'
    )
