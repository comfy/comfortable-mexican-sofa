...
class FieldStringTagTest < ActiveSupport::TestCase
  ...
  def test_initialize_tag
    assert tag = ComfortableMexicanSofa::Tag::FieldString.initialize_tag(
      cms_page_contents(:default), '{{ cms:field:content:string }}'
    )
    ...
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::FieldString.initialize_tag(
      cms_page_contents(:default), '{{cms:field:content}}'
    )
    assert_equal 'content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::FieldString.initialize_tag(
      cms_page_contents(:default), '{{cms:field:dash-content}}'
    )
    assert_equal 'dash-content', tag.identifier
    assert tag = ComfortableMexicanSofa::Tag::FieldString.initialize_tag(
      cms_page_contents(:default), '{{cms:field:namespace.content}}'
    )
