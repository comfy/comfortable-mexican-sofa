module ComfortableMexicanSofa::HasRevisions
  ...
  module InstanceMethods
    ...
    # Assigning whatever is found in revision data and attempt to save the object.
    def restore_from_revision(revision)
      # Make sure revision type matches the current record type. Also check the
      # type of the superclass because for example translations are implemented
      # via a abstract class.
      return unless revision.record == self || revision.record.class == self.class.superclass
